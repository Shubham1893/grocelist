...
export const registerNewFamily = async (req, res) => {
  const { name, email, password, familyName, mobileNumber } = req.body;

  try {
    if (!name || !email || !password || !familyName || !mobileNumber) {
      return res.status(400).json({ message: "All fields are required." });
    }
    ...
    const hashedPassword = await bcrypt.hash(password, 12);
    const newUser = await User.create({ name, email, password: hashedPassword, mobileNumber, familyId: newFamily._id });
    ...
    // --- THIS IS THE FIX ---
    console.error("REGISTRATION CRASHED! Here is the error:", error);
    // -----------------------
    res.status(500).json({ message: "Server error during new family registration.", error: error.message });
  }
};
...
export const registerJoinFamily = async (req, res) => {
  const { name, email, password, familyCode, mobileNumber } = req.body;

  try {
    if (!name || !email || !password || !familyCode || !mobileNumber) {
      return res.status(400).json({ message: "All fields are required." });
    }
    ...
    const hashedPassword = await bcrypt.hash(password, 12);
    const newUser = await User.create({ name, email, password: hashedPassword, mobileNumber, familyId: family._id });
...
export const loginUser = async (req, res) => {
    ...
    try {
        ...
        console.error("LOGIN CRASHED! Here is the error:", error);
